...
class BoundaryHandlerBase(object):
    ...
    def amend_bounds_for_integer_variables(self, integer_indices, at=0.5, offset=1e-9):
        ...
        def set_bounds(which, idx, integer_indices):
            ...
            if not np.any(idx):
                return
            dimension = max((len(self.bounds[which]), np.max(np.nonzero(idx)[0]) + 1))
    ...
    def repair(self, x, copy_if_changed=True):
        ...
        for ib in [0, 1]:
            ...
            for i in rglen(x):
                ...
                if (
                    self.bounds[ib][idx] is not None
                    and (-1) ** ib * x[i] < (-1) ** ib * self.bounds[ib][idx]
                ):
                    if copy:
    ...
    def has_bounds(self, which="both"):
        ...
        if which not in valid_whichs:
            raise ValueError(
                "`which` parameter must be in {0} but was ={1}".format(
                    valid_whichs, which
                )
            )
        bounds = self.bounds
        if (
            bounds is None
            or bounds in (False, [], ())
            or (bounds[0] is None and bounds[1] is None)
        ):
            return False
        for ib, bound in enumerate(bounds):
            if (
                bound is None
                or (ib == 0 and which == "upper")
                or (ib == 1 and which == "lower")
            ):
                continue
            sign_ = 2 * ib - 1
        ...
        for ib in [0, 1]:
            ...
            for i in rglen(x):
                if i == len(self.bounds[ib]) - 1 and (
                    self.bounds[ib][i] is None
                    or (-1) ** (1 - ib) * self.bounds[ib][i] == np.inf
                ):
                    break
                idx = min([i, len(self.bounds[ib]) - 1])
    ...
    def to_dim_times_two(self, bounds=None):
        ...
        if (
            l[0] != l[1] and 1 not in l and None not in (bounds[0][-1], bounds[1][-1])
        ):  # warn on different lengths
            _warnings.warn(
        ...
        try:
            ...
            print(
                "boundaries must be provided in the form "
...
class BoundTransform(BoundaryHandlerBase):
    ...
    def inverse(self, x, copy_if_changed=True):
        ...
        if self.bounds is None or (self.bounds[0] is None and self.bounds[1] is None):
            return x
        return np.asarray(
            self.bounds_tf.inverse(x, copy_if_changed)
        )  # this doesn't exist
    ...
    **todo**: implement a more generic interface, where this becomes a
    ...
    def update(self, function_values, es):
        ...
        if dmean.any() and (not self.weights_initialized or es.countiter == 2):  # TODO
            self.gamma = np.array(
                N * [2 * dfit]
            )  ## BUGBUGzzzz: N should be phenotypic (bounds are in phenotype), but is genotypic
        ...
        if self.weights_initialized:
            ...
            if 1 < 3:  # this is better, around a factor of two
                ...
                r = self.feasible_ratio(
                    solutions
                )  # has to be the averaged over N iterations
                self.gamma *= np.exp(np.max([N * [0], 0.3 - r], axis=0)) ** min(
                    1, es.sp.weights.mueff / 10 / N
                )
