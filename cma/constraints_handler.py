...
from .logger import (
    Logger as _Logger,
)  # we can assign _Logger = cma.logger.LoggerDummy to turn off logging
...
class ConstrainedSolutionsArchive(object):
    ...
    def __init__(self, aggregator=_g_pos_sum):
        ...
        try:
            ...
            m = (
                "``import moarchiving`` failed, hence convergence tracking "
...
class PopulationEvaluator(object):
    ...
    def __call__(self, X, **kwargs):
        ...
        try:  # use constraints.true_g attribute if available
            ...
            if (
                self.offset_from_true_g
            ):  # process true g-values and offset g-values if necessary
                for j in range(len(self.G[0])):  # for each constraint
                    ...
                    for i in range(len(self.G)):  # compute offset from all candidates
                        if self.G_true[i][j] > 0 and self.G[i][j] + offset < 0:
                            offset = -self.G[i][
                                j
                            ]  # use smallest negative value of infeasible solution
                            assert offset >= 0
                    for i in range(
                        len(self.G)
                    ):  # add offset on each infeasible candidate
                        if self.G_true[i][j] > 0:
                            self.G[i][j] += offset
                assert np.all(
                    [
                        np.all(
                            np.asarray(self.G[i])[np.asarray(self.G_true[i]) > 0] >= 0
                        )
        ...
        for name, x in kwargs.items():
            setattr(
                self, name, {"x": x, "f": self.objective(x), "g": self.constraints(x)}
            )
        return self
        ...
        for i, logger in enumerate(self):
            ...
            with _warnings.catch_warnings():  # catch misfiring add_subplot warning
                _warnings.filterwarnings(
                    "ignore", message="Adding an axes using the same arguments"
                )
                plt.subplot(2, 2, i + 1)
            logger.plot()
            if len(logger.data.shape) > 1 and logger.data.shape[1] > 1:
                for j, d in enumerate(logger.data.T):
                    ...
                    if i == 1:  # plot rolling average
                        plt.plot(
                            moving_average(d, min((moving_window_width, len(d)))),
                            color="r",
                            linewidth=0.15,
                        )
                if i == 0:
                    ...
                    if matplotlib.__version__[:3] < "3.3":
                        ...
                        plt.yscale(
                            "symlog", linthreshy=min_val
                        )  # see matplotlib.scale.SymmetricalLogScale
                    else:
...
def _log_lam(s):
    ...
    v = np.log10(
        np.maximum(1e-9, np.abs(s.lam - (0 if s.lam_opt is None else s.lam_opt)))
    )
...
class CountLastSameChanges(object):
    ...
    def init(self, dimension, force=False):
        ...
        if (
            force or self.chi_exponent_threshold is None
        ):  # threshold for number of consecutive changes
        ...
        if force or self.chi_exponent_factor is None:  # factor used in shape_exponent
            ...
            self.chi_exponent_factor = (
                4 / self.chi_exponent_threshold
            )  # steepness of ramp up
        return self
    ...
    def shape_exponent(self, i):
        ...
        return (
            self.chi_exponent_factor * s * min((self.chi_exponent_threshold, s * d))
        )  # bound change
...
class AugmentedLagrangian(object):
    ...
    def __init__(self, dimension, equality=False):
        ...
        self.g_all = _collections.defaultdict(
            _functools.partial(DequeCDF, maxlen=2 * self.dimension + 20)
        )
    ...
    def _set_parameters(self):
        ...
        if self.algorithm <= 0:  # default is 3
            ...
            raise ValueError(
                "Algorithm id {0} is not recognized".format(self.algorithm)
            )
    ...
    def _init_(self):
        ...
        if self.logging > 0:
            ...
            self.loggers.append(
                _Logger(
                    self, callables=[_log_mu], labels=["lg(mu)"], name="outauglagmu"
                )
            )
            self.logger_mu_conditions = None
            if self.algorithm in (1, 2):
                self.logger_mu_conditions = _Logger(
                    "mu_conditions",
                    labels=[
                        r"$\mu$ increases",
    ...
    def set_coefficients(self, F, G):
        ...
        if self.lam is None:  # destroys all coefficients
            ...
            self.lam_old, self.mu_old = (
                self.lam[:],
                self.mu[:],
            )  # in case the user didn't mean to
            self._initialized = np.array(self.m * [self._initialized])
        self._check_dtypes()
        idx = _and(
            _or(_not(self._initialized), self.mu == 0),
        ...
        if np.any(idx):
            ...
            dG2 = np.asarray(
                [_Mh.iqr(g**2) for g in G]
            )  # this simpler in later indexing
            if np.any(df == 0) or np.any(dG == 0) or np.any(dG2 == 0):
                _warnings.warn(
                    "iqr(f), iqr(G), iqr(G**2)) == %s, %s, %s"
            ...
            mu_new = (
                2.0 / 5 * df / self.dimension / (dG + 1e-6 * dG2 + 1e-11 * (df + 1))
            )  # TODO: totally out of thin air
            idx_inequ = _and(idx, _not(self.isequality))
            if np.any(idx_inequ):
                self.lam[idx_inequ] = df / (
                    self.dimension * dG[idx_inequ] + 1e-11 * (df + 1)
                )
            ...
            idx1 = _and(
                _and(
                    _or(isclose, _and(_not(self.isequality), sign_average <= 0.2)),
            ...
            idx2 = _and(
                _and(
                    _and(_not(isclose), _or(self.isequality, sign_average > 0.2)),
                    self.mu < mu_new,
                ),
                idx,
            )  # only increase
    ...
    def __call__(self, g):
        ...
        if len(g) != len(self.lam):
            raise ValueError(
                "len(g) = %d != %d = # of Lagrange coefficients"
                % (len(g), len(self.lam))
            )
        ...
        self.count_calls += 1
        return [
            self.lam[i] * g[i] + 0.5 * self.mu[i] * g[i] ** 2 for i in range(len(g))
        ]
    ...
    def muplus4(self, g, i, threshold=0.95):
        """return `True` if mu should be increased, else `False`"""
        if (
            self.g_all[i].cdf1(len_=2 * self.dimension) == 0
        ):  # all samples were infeasible
            return True
        if g[i] * self.mu[i] > -self.lam[i]:  # in penalized but feasible domain
            p_feas = self.g_history.cdf(i, len_=self.mucdf3_horizon)
            if p_feas > threshold:
    ...
    def update(self, f, g):
        ...
        if self.lam is None:
            ...
            if self._count_noupdate % (1 + self._count_noupdate**0.5) < 1:
                _warnings.warn(
                    "no update for %d calls (`lam` and `mu` need "
        ...
        if self.m == 0:
            return
        self._check_dtypes()
        if self.g is not None and np.any(
            self.mu > 0
        ):  # mu==0 makes a zero update anyway
            assert len(self.lam) == len(self.mu) == len(g)
            dg = np.asarray(g) - self.g
            dh = (
                f + sum(self(g)) - self.f - sum(self(self.g))
            )  # using the same coefficients
            self.count_mu_last_same_changes.init(len(self.mu))
            for i in range(len(self.lam)):
                if (
                    self.logging > 0
                    and not (self.count + 1) % self.logging
                    and self.logger_mu_conditions
                ):
                    condk1 = bool(
                        self.mu[i] * g[i] ** 2 < self.k1 * np.abs(dh) / self.dimension
                    )
                    condk2 = bool(self.k2 * np.abs(dg[i]) < np.abs(self.g[i]))
                    self.logger_mu_conditions.add(
                        i
                        - 0.1
                        + 0.25
                        * np.asarray(
                ...
                if (
                    self.isequality[i]
                    or g[i] * self.mu[i] > -self.lam[i]
                    or (
                        not utils.is_(self.count_g_in_penalized_domain)
                        or self.count_g_in_penalized_domain[i] > 0
                    )
                ):
                ...
                # mu update
                if self.algorithm == 1:
                    if self.mu[i] * g[i] ** 2 < self.k1 * np.abs(
                        dh
                    ) / self.dimension or (
                        self.k2 * np.abs(dg[i]) < np.abs(self.g[i])
                    ):  # this condition is always true if constraint i is not active
                        self.mu[i] *= (
                            self.chi_domega**0.25
                        )  # 4 / 1 is the stationary odds ratio of increment / decrement
                    else:
                    ...
                    if self.muplus3(g, i):
                        self.mu[i] *= self.chi_domega ** (
                            ...
                            * (
                                1
                                + self.count_mu_last_same_changes.update(
                                    i, 1
                                ).shape_exponent(i)
                            )
                        )
                    ...
                    raise NotImplementedError(
                        "algorithm number {0} is not known".format(self.algorithm)
                    )
            self.count += 1
...
class ConstrainedFitnessAL(object):
    ...
    def __init__(
        ...
        self.find_feasible_aggregator = (
            _g_pos_squared_sum  # sum(max(0, g)^2) = 2 x (g_al - lam x g) / mu if g > 0
        )
        ...
        try:  # treat archives as list of constraints aggregation functions
            ...
            if archives:
                self.archives = [
                    ConstrainedSolutionsArchive(fun)
                    for fun in ConstrainedFitnessAL.archive_aggregators
                ]
            else:
                self.archives = []
    ...
    def find_feasible(
        self, es, termination=("maxiter", "maxfevals"), aggregator=None
    ):  # es: OOOptimizer, find_feasible -> solution
        ...
        self(
            es.result.xfavorite
        )  # register solution, check feasibility and update best
        if self.finding_feasible:  # was set back to False when xfavorite was feasible
            ...
            while self.finding_feasible and not any(
                any(d == m for m in termination) for d in es.stop()
            ):
                X = es.ask()
                es.tell(X, [self(x) for x in X])
                self.update(
                    es
                )  # calls fun if finding_feasible finished and method is not 'best'
        ...
        if self.best_feas.x is None or self.finding_feasible:
            _warnings.warn(
                "ConstrainedFitnessAL.find_feasible: "
                " No {0}feasible solution found, stop() == {1}".format(
                    "new " if self.best_feas.x is not None else "", es.stop()
                )
            )
        assert self.best_feas.x is not None or self.finding_feasible, (
            self.best_feas,
            self.finding_feasible,
        )
    ...
    def _best_fg(self):
        ...
        i = np.nanargmin(
            self.F_plus_sum_al_G
        )  # raises ValueError when all values are nan
    ...
    def _fg_values(self, es):
        ...
        if self.which == "mean" or self.which == "solution":
            self(
                self.get_solution(es)
            )  # also update best and in case reset finding_feasible
    ...
    def update(self, es):
        ...
        if not len(self.F) == len(self.G) == len(self.F_plus_sum_al_G):
            _warnings.warn(
                "len(F, G, F_plus_sum_al_G) = ({0}, {1}, {2}) differ."
        ...
        if (
            len(self.G) == 0
        ):  # TODO: we could first self(es.result.xfavorite) and should be fine
