...
class IntegerCentering(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not 0 <= self.params["correct_bias"] <= 1:
            ...
            _warnings.warn(
                "correct_bias should be in [0, 1],"
                " the given value {0} is now interpreted as {1}".format(
        ...
        try:
            ...
            raise ValueError(
                "`method` argument must be 1 or 2, was: {0}".format(method)
            )
    ...
    def has_bounds(self, which="both"):
        ...
        try:
            ...
            _warnings.warn(
                "``bh = self.params['es'].boundary_handler`` failed with"
                "\n\n  {0}".format(e)
            )
        try:
    ...
    def lower_bounds(self, dimension):
        ...
        if self._lower_bounds is None:
            self._lower_bounds = self._set_bounds(
                "lower", np.zeros(dimension) - np.inf
            )  # np.asarray is slower than np.zeros for dimension >30
    ...
    def upper_bounds(self, dimension):
        ...
        if self._upper_bounds is None:
            self._upper_bounds = self._set_bounds("upper", np.zeros(dimension) + np.inf)
    ...
    def _set_bounds(self, which, bounds):
        ...
        if self.has_bounds():
            bounds[:] = self.params["es"].boundary_handler.get_bounds(
                which, len(bounds)
            )
            if self.params["bounds_offset"]:
                ...
                bounds[idx] += (
                    (1 if which == "lower" else -1)
                    * self.params["bounds_offset"]
                    * np.maximum(1, np.abs(bounds[idx]))
                )
        return bounds
    ...
    def get_int_indices(self, es_opts_indices=None):
        ...
        if es_opts_indices is None:  # check self.int_indices and self.params
            return (
                self.int_indices
                if (hasattr(self, "int_indices") and self.int_indices is not None)
    ...
    def callback(self, es):
        ...
        es.mean = np.dot(
            es.sp.weights.positive_weights, es.pop_sorted[0 : es.sp.weights.mu]
        )
    ...
    def __call__(self, solution_list, mean):
        ...
        if self._int_mask is None:
            self._int_mask = np.asarray(
                [i in self.int_indices for i in range(len(mean))]
            )
    ...
    def method1(self, solution_list, mean):
        ...
        for x in solution_list:
            for i in self.int_indices:
                if np.round(x[i]) != m_int[i] or (
                    self.params["correct_bias"]
                    and (
                        maxmut_ratio[i] > 0.5  # reduce bias
                        or np.random.rand()
                        < maxmut_ratio[i] / (1 - mutated_up[i] - mutated_down[i])
                    )
                ):
                    if self._record:
                        self.mahalanobis0.append(
                            self.params["es"].mahalanobis_norm(x - mean)
                        )
                    x[i] = np.round(x[i])
                    if self._record:
                        self.mahalanobis1.append(
                            self.params["es"].mahalanobis_norm(x - mean)
                        )
        ...
        if self.params["correct_bias"]:
            ...
            if _time_code:  # compare two versions
                ...
                try:
                    ...
                    self.alpha_timings = collections.defaultdict(float)
            ...
            for moves in [mpos, mneg]:
                ...
                assert (
                    moves is mneg
                    or np.random.rand() < 0.98
                    or (np.all(idx + (biases * mneg < 0) < 2))
                ), (idx, biases, mpos, mneg)
            ...
            m_int_repaired = (
                self.repair([np.array(m_int, copy=True)])[0] if repair_int else m_int
            )
            for x in solution_list:
                x += (
                    (np.round(x) == m_int)
    ...
    def repair(self, solution_list, randomized=True):
        ...
        for i, x in enumerate(solution_list[:]):
            ...
            if randomized:
                x_int = np.round(x)
                idx = np.logical_or(
                    np.logical_and(islow, x_int > x), np.logical_and(ishigh, x_int < x)
                )
                if np.any(idx):
