...
class CMAAdaptSigmaBase(object):
    ...
    def initialize_base(self, es):
        ...
        self.cs = (
            1.0
            * (es.sp.weights.mueff + 2) ** b
            / (es.N**b + (es.sp.weights.mueff + 3) ** b)
        )
    ...
    def _update_ps(self, es):
        ...
        try:
            if es.countiter <= es.sm.itereigenupdated:
                ...
                utils.print_warning(
                    "distribution transformation (B and D) have been updated before ps could be computed",
                    "_update_ps",
                    "CMAAdaptSigmaBase",
    ...
    def hsig(self, es):
        ...
        if es.opts.get(
            "CSA_invariant_path"
        ):  # was pc_for_ps = 'pc for ps' in es.opts['vv']
        ...
        return squared_sum / es.N - 1 < 1 + 4.0 / (es.N + 1)
...
class CMAAdaptSigmaDistanceProportional(CMAAdaptSigmaBase):
    ...
    def __init__(self, coefficient=1.2, **kwargs):
        ...
        super(
            CMAAdaptSigmaDistanceProportional, self
        ).__init__()  # base class provides method hsig()
    ...
    def update(self, es, **kwargs):
        ...
        """
        es.sigma = (
            self.coefficient * es.sp.weights.mueff * _norm(es.mean) / es.N / es.sp.cmean
        )
...
class CMAAdaptSigmaCSA(CMAAdaptSigmaBase):
    ...
    def initialize(self, es):
        ...
        """
        self.disregard_length_setting = (
            True if es.opts["CSA_disregard_length"] else False
        )
        if es.opts["CSA_clip_length_value"] is not None:
            try:
                if len(es.opts["CSA_clip_length_value"]) == 0:
                    ...
                    es.opts["CSA_clip_length_value"] = [
                        -np.inf,
                        es.opts["CSA_clip_length_value"][0],
                    ]
                elif len(es.opts["CSA_clip_length_value"]) == 2:
                    es.opts["CSA_clip_length_value"] = np.sort(
                        es.opts["CSA_clip_length_value"]
                    )
                else:
                    raise ValueError(
                        "option CSA_clip_length_value should be a number of len(.) in [1,2]"
                    )
            ...
            es.opts["CSA_clip_length_value"] = list(
                np.sort(es.opts["CSA_clip_length_value"])
            )
            if (
                es.opts["CSA_clip_length_value"][0] > 0
                or es.opts["CSA_clip_length_value"][1] < 0
            ):
                raise ValueError(
                    "option CSA_clip_length_value must be a single positive or a negative and a positive number"
                )
        ## meta_parameters.cs_exponent == 1.0
        b = 1.0
        ## meta_parameters.cs_multiplier == 1.0
        ...
        if csa_dampdown_fac != 1 and es.opts["verbose"] > 1:
            print(
                "CSA damping is asymmetric: dampdown = {0} x dampup".format(
                    csa_dampdown_fac
                )
            )
        self.max_delta_log_sigma = (
            1  # in symmetric use (strict lower bound is -cs/damps anyway)
        )
        ...
        if self.disregard_length_setting:
            ...
            self.cs = (
                1.0
                * (es.sp.weights.mueff + 1) ** b
                / (es.N**b + 2 * es.sp.weights.mueff**b)
            )
        ...
        if es.opts["CSA_clip_length_value"] is not None:
            ...
            if vals[0] > 0 or vals[1] < 0:
                raise ValueError(
                  ...
                  not allowed"""
                    % str(es.opts["CSA_clip_length_value"])
                )
    ...
    def update2(self, es, **kwargs):
        ...
        self._update_ps(
            es
        )  # caveat: if es.B or es.D are already updated and ps is not, this goes wrong!
        p = self.ps
        if es.opts.get(
            "CSA_invariant_path"
        ):  # was pc_for_ps = 'pc for ps' in es.opts['vv']
...
class CMAAdaptSigmaMedianImprovement(CMAAdaptSigmaBase):
    ...
    def initialize(self, es):
        ...
        self.index_to_compare = (
            0.5 * (r**0.5 + 2.0 * (1 - r**0.5) / np.log(es.N + 9) ** 2) * (es.popsize)
        )  # TODO
    ...
    def update(self, es, **kwargs):
        if es.countiter < 2:
            ...
            ft1, ft2 = (
                self.fit[int(self.index_to_compare)],
                self.fit[int(np.ceil(self.index_to_compare))],
            )
            ftt1, ftt2 = (
                es.fit.fit[(es.popsize - 1) // 2],
                es.fit.fit[int(np.ceil((es.popsize - 1) / 2))],
            )
            ...
            if 1 < 3:
                s += pt2 * sum(
                    es.fit.fit <= self.fit[int(np.ceil(self.index_to_compare))]
                )
                s += (1 - pt2) * sum(es.fit.fit < self.fit[int(self.index_to_compare)])
                s -= es.popsize / 2.0
                s *= 2.0 / es.popsize  # the range was popsize, is 2
...
class CMAAdaptSigmaTPA(CMAAdaptSigmaBase):
    ...
    def initialize(self, N=None, opts=None):
        ...
        try:
            damp_fac = opts[
                "CSA_dampfac"
            ]  # should be renamed to sigma_adapt_dampfac or something
        except (TypeError, KeyError):
            damp_fac = 1

        self.sp = (
            utils.BlancClass()
        )  # just a container to have sp.name instead of sp['name'] to access parameters
        try:
            self.sp.damp = (
                damp_fac * eval("N") ** 0.5
            )  # (1) why do we need 10 <-> np.exp(1/10) == 1.1? 2 should be fine!?
            self.sp.damp = damp_fac * (
                4 - 3.6 / eval("N") ** 0.5
            )  # (2) should become new default!?
            self.sp.damp = damp_fac * eval("N") ** 0.25
            self.sp.damp = 0.7 + np.log(
                eval("N")
            )  # between 2 and 9 very close to N**1/2, for N=7 equal to (1) and (2)
        ...
        if tpa_dampdown_fac != 1 and (opts is None or opts.get("verbose", 0) > 1):
            print(
                "TPA damping is asymmetric: dampdown = {0} x dampup".format(
                    tpa_dampdown_fac
                )
            )
        ...
        self.sp.sigma_fac = (
            1.0  # (obsolete) 0.5 feels better, but no evidence whether it is
        )
    ...
    def update(self, es, function_values, **kwargs):
        ...
        if self.initialized is not True:
            utils.print_warning(
                "dimension not known, damping set to 4", "update", "CMAAdaptSigmaTPA"
            )
            self.initialized = True
        if 1 < 3:
        ...
        self.s = (1 - self.sp.c) * self.s + self.sp.c * np.sign(z) * np.abs(
            z
        ) ** self.sp.z_exponent
        # self.s2 = (1 - self.sp.c) * self.s2 + self.sp.c * np.sign(z) * np.abs(z)**(2*self.sp.z_exponent)
        es.sigma *= np.exp(
            self.s / self.sp.damp / (tpa_dampdown_fac if self.s < 0 else 1)
        )
            ...
            for i in np.random.randint(0, es.N, 1):
                ...
                if (
                    ...
                    * (es.pop[1][i] - es.mean[i])
                    ...
                    expected_precision = 1e-11 * max(
                        (
                            1,
                            np.abs(es.mean[i]) / es.stds[i],
                            np.abs(es.mean[j]) / es.stds[j],
                        )
                    )
                    # was: expected_precision = 1e-4
                    if not Mh.equals_approximately(
                        dmi_div_dx0i, dm / dx0, expected_precision
                    ) or not Mh.equals_approximately(
                        ...
                        m and _warnings.warn(m)
                else:
                    m = utils.format_warning(
                        "zero delta encountered in TPA which"
